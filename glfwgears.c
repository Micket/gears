/*
 * This is a port of "glxgears" demo to GLFW
 *
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    if (key == GLFW_KEY_LEFT) {
        ...
        animate = !animate;
        printf("changing animation state");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    window = glfwCreateWindow(100, 100, "glfwgears", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    //EGLDisplay egl_dpy = glfwGetEGLDisplay();
    //EGLContext egl_ctx = glfwGetEGLContext();
    
    if (printInfo) {
        int major = glfwGetWindowAttrib(window, GLFW_CONTEXT_VERSION_MAJOR);
        int minor = glfwGetWindowAttrib(window, GLFW_CONTEXT_VERSION_MINOR);
        int ctxtype = glfwGetWindowAttrib(window, GLFW_CONTEXT_CREATION_API);
        char *ctxname;
        if (ctxtype == GLFW_NATIVE_CONTEXT_API) {
            ctxname = "Native";
        } else if (ctxtype == GLFW_EGL_CONTEXT_API) {
            ctxname = "EGL";
        } else if (ctxtype == GLFW_OSMESA_CONTEXT_API) {
            ctxname = "OSMESA";
        } else {
            ctxname = "unknown";
        }
        //printf("glfwgears: EGL_VENDOR = %s\n", eglQueryString(egl_dpy, EGL_VENDOR));
        
        printf("Context = %s v%d.%d\n", ctxname, major, minor);
